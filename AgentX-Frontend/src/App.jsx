        ...
        <div className="bg-[#0D0F10] rounded-[30px]">
          ...
          <header className="flex items-center justify-between px-8 py-10">
            <div>
              ...
              <p className="text-sm mt-1 text-[#9B9C9E] plus-jakarta-sans-500">Launch gaming agents in secs</p>
            </div>
            <div className="flex items-center">
              <div className="w-[38px] h-[38px] rounded mr-5 p-1">
              <img src={profile} alt="" className="w-full h-full rounded"/>
              </div>
          ...
          <nav className="flex px-8 pt-4">
            {tabs.map((e)=>{
              return <NavButton active={e.number===currentTab} onClick={()=>{setCurrentTab(e.number)}} key={e.number} text={e.text} logo={e.logo} activelogo={e.activelogo}/>
            })}
